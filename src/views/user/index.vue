  ...
  <div class="user-page">
    ...
    <div class="page-header">
      <a-breadcrumb>
        ...
        <a-breadcrumb-item>Restful CRUD</a-breadcrumb-item>
      </a-breadcrumb>
    ...
    <!-- 主内容卡片 -->
    <a-card class="content-card" title="Restful CRUD">
      <!-- 搜索表单 -->
        ...
        <template #action="{ record }">
          <a-space>
            ...
            <a-button
              size="mini"
              type="secondary"
              :disabled="!hasButton('user:modify-role')"
    ...
    <RoleDialog
      ...
      :user-id="currentUserId"
      :current-role-ids="currentRoleIds"
...
const detailDrawerAction = ref<'add' | 'edit'>('add')
const currentUserId = ref('')

// 角色对话框
const roleDialogVisible = ref(false)
const currentRoleIds = ref<string[]>([])
...
const addToQuickMenu = () => {
  ...
  const titleText = 'Restful CRUD'
  const current = { key: '/system/user', name: titleText }
...
// 分配角色
const handleAssignRole = (record: UserQueryModel) => {
  currentUserId.value = record.id
  currentRoleIds.value = record.roles.map((r) => r.id)
